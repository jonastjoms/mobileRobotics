...
def rule_based(front, front_left, front_right, left, right):
  ...
  # If not stuck run same u if not too close to obstacles
  u = 0.8
  if (left < right):
      w = -0.2
  else:
  ...
  if np.min([front_left,front_right, left, right, front]) < 0.5:
      u = 0.4
  # If stuck
  if front < np.min([front_left,front_right, left, right]):
      u = -0.5
      w = front_right + right -front_left - left
