...
def sample_random_position(occupancy_grid):
  position = np.array([np.random.uniform(MINPOS,MAXPOS), np.random.uniform(MINPOS,MAXPOS)], dtype=np.float32)
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  # Get vector between the two nodes
  v_between = final_position-node.pose[0:2]
  # Find angle between the direction of the first node and the vector v_between
  alpha = node.pose[YAW] - np.arctan2(v_between[Y], v_between[X])
  # The new nodes rotation should now be the previous nodes rotation - 2*alpha
  final_node.pose[YAW]-= 2*alpha
  # Check whether circle is valid:
  center, radius = find_circle(node, final_node)
  # Find the angle between the two vectors going from the center of the circle to the two nodes
  v_center_node1 = node.pose[0:2]-center
  v_center_node2 = final_position-center
  ...
  angles = np.arange(angle_node2, angle_node1, 0.0005)
  is_valid = True
  for angle in angles:
      x = radius*np.cos(angle)
      y = radius*np.sin(angle)
      if not occupancy_grid.is_free(center + np.array((x,y))):
          is_valid = False
          break
  if not is_valid:
      final_node = None
