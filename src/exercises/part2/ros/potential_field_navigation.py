...
def get_relative_position(absolute_pose, absolute_position):
  ...
  rel_pos_world = absolute_position - absolute_pose[0:2]
  # Need relative position in robot frame, use a rotational matrix
  rot = np.array(((np.cos(absolute_pose[YAW]), np.sin(absolute_pose[YAW])),(-np.sin(absolute_pose[YAW]), np.cos(absolute_pose[YAW]))))
  relative_position = np.matmul(rot,rel_pos_world)
